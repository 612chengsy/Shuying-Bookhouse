...
export const BookReader: React.FC<BookReaderProps> = ({
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'cover' | 'backCover') => {
    if (!isAdmin) {
    ...
    try {
      ...
      if (serverUrl && onUpdateCoverImage) {
        if (target === 'cover') {
          onUpdateCoverImage(book.id, serverUrl, book.backCoverImage);
          setUploadMsg('封面图片已成功上传并关联后端服务器！');
        } else {
          onUpdateCoverImage(book.id, book.coverImage || '', serverUrl);
          setUploadMsg('封底图片已成功上传并关联后端服务器！');
        }
