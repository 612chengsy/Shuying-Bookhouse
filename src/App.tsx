...
export default function App() {
  ...
  // Increment view count on initial site visit
  useEffect(() => {
    setTotalViews(v => v + 1);
  }, []);
  ...
  const handleOpenReader = (book: Book) => {
    ...
    setBooks(prev => prev.map(b => b.id === book.id ? { ...b, views: b.views + 1 } : b));
    setTotalViews(v => v + 1);
  ...
  const handleToggleLikeBook = (bookId: string) => {
    ...
    if (isLiked) {
      ...
      setBooks(prev => prev.map(b => b.id === bookId ? { ...b, likes: Math.max(0, b.likes - 1) } : b));
    } else {
  ...
  const handleAddChapter = (bookId: string, chapterTitle: string, chapterContent: string) => {
    ...
    setBooks(prev => prev.map(b => {
      if (b.id === bookId) {
        const updatedChapters = [...(b.chapters || []), newChapter];
        const totalChars = updatedChapters.reduce((acc, c) => acc + (c.content?.length || 0), 0);
        const wordCountStr = formatWordCount(totalChars);
        return {
          ...b,
  ...
  const handleDeleteChapter = (bookId: string, chapterId: string) => {
    ...
    setBooks(prev => prev.map(b => {
      if (b.id === bookId) {
        const updatedChapters = (b.chapters || []).filter(c => c.id !== chapterId);
        const totalChars = updatedChapters.reduce((acc, c) => acc + (c.content?.length || 0), 0);
        const wordCountStr = formatWordCount(totalChars);
        return {
          ...b,
